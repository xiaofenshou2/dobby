...
NDK_HOME="$NDK_BASE/r25c"
if [ ! -d "$NDK_HOME" ]; then
  echo "警告: r25c 精确路径不存在，尝试寻找 r25*"
  NDK_HOME=$(ls -d $NDK_BASE/r25* 2>/dev/null | head -1)
fi
if [ -z "$NDK_HOME" ] || [ ! -d "$NDK_HOME" ]; then
  echo "错误: 找不到 NDK r25c！请检查 GitHub Actions 环境。"
  exit 1
fi
echo "最终使用 NDK = $NDK_HOME"
export ANDROID_NDK_HOME=$NDK_HOME

echo "[3/5] 克隆 Dobby (使用 chiteroman/master，已规避 arm64 asm 报错) ..."
